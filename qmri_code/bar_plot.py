...
#datime_list = ['2022-05-16_05-39-56']
...
#datime_now = '2022-05-25_09-54-16'
#datime_now = '2022-05-17_14-58-44'
datime_now = '2022-06-21_00-24-25'
...
for index, datime in enumerate(datime_list):
    ...
    #plt.yscale('log')
    plt.figure()
    plt.xticks(ticks = [1., 1.25, 1.5, 2., 2.25, 2.5])
    plt.bar(x = [1., 2], data = ['Chi', 'Gauss'], height = [-sum(mt_chi), -sum(mt_gauss)], color = 'b', align = 'center', width = 0.2)
    #plt.xticks( ticks = [1., 2], labels = None)
    plt.bar(x = [1.25, 2.25],  data = ['Chi', 'Gauss'], height = [-sum(pd_chi), -sum(pd_gauss)], color = 'g', align = 'center', width = 0.2)
    #plt.xticks(ticks = [1.25, 2.25], labels = None)
    plt.bar(x = [1.5, 2.5], data = ['Chi', 'Gauss'], height = [-sum(t1_chi), -sum(t1_gauss)], color = 'r', align = 'center', width = 0.2)
    #plt.xticks(ticks = [1.5, 2.5], labels = None)
    #plt.xticks(ticks = [1., 1.25, 1.5, 2., 2.25, 2.5], labels = None)
    
    plt.ylabel('negative loglikelihood')
    plt.title(cl_list[0])
    plt.grid(color='#95a5a6', linestyle='--', which='both', linewidth=2, axis='y', alpha=0.5)
    plt.legend(["MT", "PD", "T1"])
  
    plot_txt = 'likelihood_bar_' + datime + '_' + datime_now + '_' + cl_list[index] + '.png'
    print(plot_txt)
